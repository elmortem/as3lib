package realtimelib.session
{
	...
	{		
		...
		// member constants
		private const ANONYMOUS_USER_NAME:String = "warrior";
		...
		*/
		...
		private const DEFAULT_EXPIRE_TIME:Number = 5000;		// every 5 seconds
		private const DEFAULT_EXPIRE_TIMEOUT:Number = 20000;	// every 20 seconds
		...
		public function UserList(connection:NetConnection, groupspec:String, username:String = null, userDetails:Object=null)
		{
			...
			m_connection.addEventListener(NetStatusEvent.NET_STATUS, connectionStatusHandler);
		...
		override public function close():void {
			...
			m_connection.removeEventListener(NetStatusEvent.NET_STATUS, connectionStatusHandler);
			m_connection = null;
			
			m_keepAliveTimer.stop();
			m_keepAliveTimer.removeEventListener(TimerEvent.TIMER, announceSelf);
			m_keepAliveTimer = null;
			
			m_expireTimer.stop();
			m_expireTimer.removeEventListener(TimerEvent.TIMER, expireNames);
			m_expireTimer = null;
		...
		private function connectionStatusHandler(evt:NetStatusEvent):void
		{
			if( evt.info.code == "NetGroup.Connect.Success" )
			{
				...
				(evt.target as NetConnection).removeEventListener(NetStatusEvent.NET_STATUS, connectionStatusHandler);
