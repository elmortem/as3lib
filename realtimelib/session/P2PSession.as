...
package realtimelib.session
{
	...
	{		
		...
		public function close():void {
			connection.removeEventListener(NetStatusEvent.NET_STATUS, netStatus, false);
			connection.close();
			connection = null;
			
			if(_mainChat != null) {
				_mainChat.removeEventListener(NetStatusEvent.NET_STATUS, netStatus, false);
				_mainChat.removeEventListener(PeerStatusEvent.USER_ADDED, onUserAdd);
				_mainChat.removeEventListener(PeerStatusEvent.USER_REMOVED, onUserRemove);
				_mainChat.close();
				_mainChat = null;
			}
		...
		public function join():void {
			...
			mainChat = new GroupChat(this, getGroupSpec().groupspecWithoutAuthorizations(), myUser.name, myUser.details);
		
			mainChat.addEventListener(NetStatusEvent.NET_STATUS, netStatus, false, 0, true);
			mainChat.addEventListener(PeerStatusEvent.USER_ADDED, onUserAdd);
			mainChat.addEventListener(PeerStatusEvent.USER_REMOVED, onUserRemove);
		}
		private function onUserAdd(event:PeerStatusEvent):void {
			dispatchEvent(event);
		}
		private function onUserRemove(event:PeerStatusEvent):void {
			dispatchEvent(event);
		}
		...
		public function netStatus(e:NetStatusEvent):void{
			...
			switch(e.info.code) {
				case "NetConnection.Connect.Success":
					statusWrite("- Connected to Adobe Cirrus -");
